...
- **`video_companion`** — **`stop_clear_queue_connected`** MPD first; transport uses **SIGSTOP/SIGCONT** (pause) and **seek** restarts **`ffmpeg`** with **`-ss`**. Encoding uses **`-re`**, **4 s** HLS segments, mux queues; **ALSA** gets **`-af aresample=async=1`** so brief encode stalls do not sound like a variable-speed “tap”. **`EVO_VIDEO_ENCODER`**: default **`auto`** uses **`h264_v4l2m2m`** when **`/dev/video11`**-style nodes exist (Raspberry Pi), else **`libx264`** with capped threads; force **`libx264`** or **`h264_v4l2m2m`** if needed. Hardware encode needs the service user in **`video`** / **`render`** (**§11**).
