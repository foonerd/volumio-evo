...
**Scope:** **Compatibility-adapter reference** ([CONCEPT.md](CONCEPT.md) §6, UI compatibility surface row) for **volumio3-backend** (Node) wire compatibility. Not a product roadmap. Product architecture and vocabulary (**steward**, **fabric**, **racks**, **slots**, **plugins**) live in **[CONCEPT.md](CONCEPT.md)** - read that first. New features are declared as racks and stocked by plugins, not added to this inventory; entries here retire as consumers move to fabric projections.

This document (1) lists **all current Volumio (volumio3-backend) functionality**, (2) summarizes **what Evo already covers**, (3) records **decisions**, (4) sets **outstanding work by phase** (transport, UI compatibility, networking, plugins, system, other), and (5) states **what cannot be ported and why**.

Reference codebase: **volumio3-backend** (Node/Express). Volumio4, if present elsewhere, can be added to the inventory later.

**Product architecture** (steward, fabric, racks, slot-stocked plugins) is **[CONCEPT.md](CONCEPT.md)**. **This document** is deliberately **Node-centric in Part 1**: it inventories the legacy backend for **wire compatibility**. Today's Evo still ships most logic as a monolithic steward binary plus a WASM host; that is an implementation state, not a ceiling. Treat **CONCEPT.md** as the boundary of record.
...
### 1.5 Plugins and extensibility (Node reference)
...
**Evo target:** Extensibility matches **[CONCEPT.md](CONCEPT.md)** §3–6 — catalogue **slots**, versioned manifests, multiple **hosts** (WASM, native, helpers). **Today:** WASM guest ABI ([PLUGIN_ABI.md](PLUGIN_ABI.md)), optional **`.wasm`** in `plugin_dir`, and many concerns still **inlined** in the steward binary ([PLUGIN_CORE_VS_EXTENSIONS.md](PLUGIN_CORE_VS_EXTENSIONS.md)).
...
| callMethod (**system_controller/system**) | Partial | **save*** settings (general, locale, kiosk, updates, privacy), **`installBootBranding`** (boot installer — [BRANDED_BOOT.md](BRANDED_BOOT.md)), **`installKioskLayer`** (re-runs `layer/kiosk-wpe/install.sh` like bootstrap `--kiosk-wpe`); ALSA/MPD paths under **audio_interface/** and **music_service/** — see `socketio.rs` |
...
| getSleep, setSleep, getAlarms, saveAlarm | Yes | Sleep timer + daily alarms persist under **`settings/alarm/`** ([SETTINGS_LAYOUT.md](SETTINGS_LAYOUT.md)); **`rtcwake`** integration [ALARM_WAKE.md](ALARM_WAKE.md). **`pushSleep`** / **`pushAlarm`** return real state (appointments-shaped behaviour per [CONCEPT.md](CONCEPT.md) §6). |
| getMultiroom, setMultiroom, writeMultiroom | Yes (stub/no-op) | getMultiroom/setMultiroom -> pushMultiroom `{}`; writeMultiroom no-op. |
| getExtendedOutputDevices, getOutputDevices | Yes | **`pushExtendedOutputDevices`** / **`pushOutputDevices`** — card list from **`aplay -l`**, **`cards.json`** prettynames / I2S filtering, **`dacs.json`** when I2S on ([SETTINGS_LAYOUT.md](SETTINGS_LAYOUT.md)). |
...
| setOutputDevices | Yes | Same contract as **`saveAlsaOptions`**: persist **`settings/alsa/state.toml`**, optional Pi **`dtoverlay`**, MPD fragment restart; may **broadcast** **`openModal`** (Restart) when **`dacs.json`** **`needsreboot`** and I2S DAC **selection changed** (mirrors Node **`alsa_controller`**). |
...
|----------|---------|------------|--------|
| **List installed** | getInstalledPlugins → pushInstalledPlugins (Node plugins) | **Implemented** | **Today:** lists **`.wasm`** in `plugin_dir`; same Socket.IO name, simplified payload. **Target:** manifests + slot admission ([CONCEPT.md](CONCEPT.md) §5–6). |
| **Plugin store / catalog** | getAvailablePlugins, getPluginDetails (from Volumio store/cloud) | **Not ported** | Legacy store targets **Node** npm packages. A future Evo catalogue would distribute **fabric** artefacts (not 1:1 with Node). |
| **Install / update / uninstall** | installPlugin, updatePlugin, unInstallPlugin (zip from store or upload) | **Not ported** | Node: npm/zip. Evo **today:** manual **`.wasm`** drop / config. **Target:** versioned plugin bundles + steward lifecycle ([CONCEPT.md](CONCEPT.md) §4–5). |
| **Enable / disable / status** | enablePlugin, disablePlugin, modifyPluginStatus, preUninstallPlugin | **Not ported** | Node: per-plugin config flags. Evo: lifecycle **deferred** pending manifest/schema ([CONCEPT.md](CONCEPT.md) §9). |
| **Generic plugin RPC** | callMethod(endpoint, method, data), pluginEndpoint REST | **Partial** | **POST /api/v1/pluginEndpoint** **`metavolumio`**; **callMethod** **`clearAlbumartCache`**, **system**/**ALSA**/**MPD** saves, **`installBootBranding`**, **`installKioskLayer`** — see **`socketio.rs`**, [BRANDED_BOOT.md](BRANDED_BOOT.md), [KIOSK.md](KIOSK.md). Broad parity **deferred** until steward/subscription/protocol work ([CONCEPT.md](CONCEPT.md) §9). |
...
**Cannot be ported from Node/npm as-is:** Part 5 (does not limit **[CONCEPT.md](CONCEPT.md)** plugin futures).
...
| **Audio outputs (extra)** | **Partial** | **Multi-zone / “audio outputs” plugin API:** getAudioOutputs, enableAudioOutput, disableAudioOutput, setAudioOutputVolume, audioOutputPlay, audioOutputPause → pushAudioOutputs — **not ported**. **Main Playback options path** (getExtendedOutputDevices, getOutputDevices, setOutputDevices, **callMethod** saveAlsaOptions) **is implemented** — see §3.1 **Playback / ALSA**. |
...
|----------------|-------------------------|
| **Node.js plugin system** | Volumio plugins are Node modules (JavaScript, npm, require). There is no 1:1 port of a **Node npm package** into Evo. Evo’s **target** model is **[CONCEPT.md](CONCEPT.md)** — slot contracts, manifests, multiple hosts (WASM is one). **Today** the repo ships a WASM ABI plus inlined modules; that is **transitional** packaging, not the final architecture. |
| **callMethod / pluginEndpoint (generic)** | These call into Node plugins by name and method. Evo has no Node runtime. **Exceptions:** **POST pluginEndpoint** **`metavolumio`**; **`callMethod`** **`clearAlbumartCache`**; **`system_controller/system`** saves, **`installBootBranding`**, **`installKioskLayer`**; **`audio_interface` / `music_service`** ALSA/MPD **`save*`** (including **`saveAlsaOptions`** → may **broadcast** **`openModal`** for I2S reboot — §3.1) — see **`socketio.rs`**, [KIOSK.md](KIOSK.md). Full generic RPC is **deferred** until projection/subscription protocols land ([CONCEPT.md](CONCEPT.md) §9). |
| **Volumio plugin store** | The store is a Volumio-hosted service for **Node** packages. Evo has no equivalent today. A future distribution channel would serve **Evo-native** artefacts per **[CONCEPT.md](CONCEPT.md) §5** (shelf, shape version, trust class) — not necessarily **`.wasm`** only. |
...
4. **What we will never port as-is:** Use Part 5 ("Cannot be ported") for **legacy Node/npm** and external service limits. It does **not** override **[CONCEPT.md](CONCEPT.md)** for Evo’s own plugin architecture.
