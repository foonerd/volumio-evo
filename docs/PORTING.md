...
- **Plugin REST:** `POST /api/v1/pluginEndpoint` is implemented for **`endpoint: "metavolumio"`** only (browse story/credits). Other plugin endpoints remain unimplemented. **callMethod** (Socket.IO) is used for album-art cache clear and similar; full plugin ABI is still future work.
...
| **Generic plugin RPC** | callMethod(endpoint, method, data), pluginEndpoint REST | **Partial** | **POST /api/v1/pluginEndpoint** implements **`metavolumio`** only (browse album/artist text + credits). **callMethod(clearAlbumartCache)** is implemented (broadcast to UI). Other plugin methods / endpoints remain **deferred** until Evo plugin ABI. |
...
| **callMethod / pluginEndpoint (generic)** | These call into Node plugins by name and method. Evo has no Node runtime and no equivalent generic RPC to Node plugins. **Exception:** **POST pluginEndpoint** with **`endpoint: "metavolumio"`** is implemented in Rust for browse UI text (not Node). Other endpoints and **callMethod** targets remain **deferred** except **clearAlbumartCache** (broadcast). Full plugin RPC is **deferred** until Evo’s plugin API is defined. |
