...
- Write **`/etc/systemd/system/volumio-evo.service.d/10-runtime-user.conf`** with `User=`, `Group=`, `SupplementaryGroups=audio`, `HOME=`, **`VOLUMIO_EVO_RUNTIME_USER=<name>`**, and **`VOLUMIO_EVO_SYSTEMCTL=<path>`** (resolved **`systemctl`** binary — must match sudoers below).
