...
| **`EVO_INSTALL_UI_BOOTSTRAP_SUDOERS`** | If `1` (default), install **`/etc/sudoers.d/volumio-evo-ui-bootstrap`** so the service user may run **`sudo -n`** **`bootstrap-volumio-evo-player.sh`** **`--apply-ui-only`** (path **`VOLUMIO_EVO_BOOTSTRAP_SCRIPT`**); nginx UI roots follow **Appearance** layout. Set `0` to skip. |
...
- Write **`/etc/systemd/system/volumio-evo.service.d/10-runtime-user.conf`** with `User=`, `Group=`, `SupplementaryGroups=audio`, `HOME=`, **`VOLUMIO_EVO_RUNTIME_USER=<name>`**, **`VOLUMIO_EVO_SYSTEMCTL=<path>`**, **`VOLUMIO_EVO_BOOTSTRAP_SCRIPT`** (canonical **`bootstrap-volumio-evo-player.sh`** path), **`VOLUMIO_EVO_HOSTNAMECTL`** / **`VOLUMIO_EVO_TIMEDATECTL`**, **`VOLUMIO_EVO_RTCWAKE`** (paths must match **`volumio-evo-ui-bootstrap`**, **`volumio-evo-hostname-timedate`**, **`volumio-evo-rtcwake`** sudoers when installed).
