...
| **Bootstrap `--apply-ui-only`** | After layout save, refresh nginx so static UI roots match **`/etc`** config | **Root**, or **`sudo -n $VOLUMIO_EVO_BOOTSTRAP_SCRIPT --apply-ui-only`** — bootstrap **`volumio-evo-ui-bootstrap`** when **`EVO_INSTALL_UI_BOOTSTRAP_SUDOERS=1`** |
