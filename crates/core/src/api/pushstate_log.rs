...
use crate::log_tags::{EVO_PUSHSTATE, EVO_STATE};
...
#[inline]
pub(crate) fn warn_broadcast_resync(err: impl std::fmt::Display) {
    tracing::warn!(
        "{} broadcast get_state_and_queue_connected failed (no pushState/pushQueue): {}",
        EVO_STATE,
        err
    );
}
